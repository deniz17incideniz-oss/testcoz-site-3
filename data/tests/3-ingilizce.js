...
(function () {
  ...
  themes["homes-houses-and-the-neighbourhood"] = themes["homes-houses-and-neighbourhoods"];
  themes["homes-houses-and-the-neighbourhood"].name = "Homes, Houses and the Neighbourhood";
  delete themes["homes-houses-and-neighbourhoods"];
  ...
  Object.keys(themes).forEach(topic=>levels.forEach(difficulty=>{
    const t=themes[topic],slug="3-sinif-ingilizce-"+topic+"-"+difficulty+"-test-1";
    const questions=makers[difficulty](t).map((x,i)=>{x.id=i+1;x.question="3. sınıf İngilizce "+difficulty+" — "+t.name+": "+x.question;if(x.visual){x.image="images/tests/"+slug+"-soru-"+(i+1)+".svg";x.imageAlt=x.visual.title;}return x;});
    tests.push({classLevel:3,subject:"ingilizce",subjectName:"İngilizce",topic,topicName:t.name,difficulty,testNumber:1,slug,pageUrl:"tests/"+slug+".html",questions});
  }));
