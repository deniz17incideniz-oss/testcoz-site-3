...
(function () {
  ...
  const grade3NumberConfigs = {
    kolay: { n: 234, m: 243, third: 241, start: 120, step: 10, target: 348 },
    orta: { n: 458, m: 485, third: 465, start: 225, step: 25, target: 572 },
    zor: { n: 706, m: 670, third: 783, start: 338, step: 40, target: 893 }
  };

  function numbersOneGrade3(difficulty) {
    const c = grade3NumberConfigs[difficulty];
    const hundreds = Math.floor(c.n / 100);
    const tens = Math.floor(c.n / 10) % 10;
    const ones = c.n % 10;
    const sorted = [c.n, c.m, c.third].slice().sort(function (a, b) { return a - b; });
    return [
      q(c.n + " sayısı kaç yüzlük, kaç onluk ve kaç birlikten oluşur?", [hundreds + " yüzlük " + tens + " onluk " + ones + " birlik", tens + " yüzlük " + hundreds + " onluk " + ones + " birlik", hundreds + " yüzlük " + ones + " onluk " + tens + " birlik", c.n + " onluk"], hundreds + " yüzlük " + tens + " onluk " + ones + " birlik", "Basamaklar sırasıyla " + hundreds + " yüzlük, " + tens + " onluk ve " + ones + " birliktir."),
      q("Görseldeki yüzlük-onluk-birlik modeli hangi sayıyı gösterir?", [String(c.n), String(hundreds + tens + ones), String(hundreds * 100 + ones * 10 + tens), String(tens * 100 + hundreds * 10 + ones)], String(c.n), "Model " + c.n + " sayısını gösterir.", visual("chart", "Basamak modeli", { items: [{ label: "Yüzlük", value: hundreds }, { label: "Onluk", value: tens }, { label: "Birlik", value: ones }] })),
      q(c.n + " ve " + c.m + " sayılarından hangisi daha büyüktür?", [String(Math.max(c.n,c.m)), String(Math.min(c.n,c.m)), String(Math.abs(c.n-c.m)), String(c.n+c.m)], String(Math.max(c.n,c.m)), "Yüzlük, ardından onluk ve birlik basamakları karşılaştırılır."),
      q(c.n + ", " + c.m + " ve " + c.third + " sayılarının küçükten büyüğe sırası hangisidir?", [sorted.join(" < "), [sorted[2],sorted[1],sorted[0]].join(" < "), [sorted[0],sorted[2],sorted[1]].join(" < "), [sorted[1],sorted[0],sorted[2]].join(" < ")], sorted.join(" < "), "Doğru sıra " + sorted.join(", ") + " biçimindedir."),
      numeric(c.start + "den başlayıp " + c.step + "er sayarken sıradaki sayı kaçtır: " + c.start + ", " + (c.start+c.step) + ", " + (c.start+2*c.step) + ", ?", c.start+3*c.step, [c.start+2*c.step+10,c.start+4*c.step,c.start+3*c.step-1], "Her adımda " + c.step + " eklenir.", visual("numberLine", "Ritmik sayma", { start:c.start,end:c.start+4*c.step,arrow:[c.start,c.start+c.step,c.start+2*c.step,c.start+3*c.step] }), 5),
      numeric(c.target + " sayısının bir eksiği ile bir fazlasının toplamı kaçtır?", c.target*2, [c.target*2-1,c.target*2+1,c.target], (c.target-1) + " + " + (c.target+1) + " = " + (c.target*2) + ".", null, 6),
      numeric(c.n + " sayısındaki " + hundreds + " rakamının basamak değeri kaçtır?", hundreds*100, [hundreds,tens*10,c.n], "Yüzler basamağındaki rakamın değeri " + (hundreds*100) + "dür.", null, 7),
      numeric("Görselde " + c.start + ", " + (c.start+c.step) + ", ?, " + (c.start+3*c.step) + " örüntüsü vardır. Eksik sayı kaçtır?", c.start+2*c.step, [c.start+c.step+1,c.start+3*c.step,c.start+4*c.step], "Örüntü " + c.step + "er artar.", visual("equation", "Eksik sayı", { text:c.start+", "+(c.start+c.step)+", ?, "+(c.start+3*c.step) }), 8),
      numeric(hundreds + " yüzlük, " + tens + " onluk ve " + ones + " birlikten oluşan sayının " + c.step + " fazlası kaçtır?", c.n+c.step, [c.n-c.step,c.n+c.step+10,hundreds+tens+ones], c.n + " + " + c.step + " = " + (c.n+c.step) + ".", null, 9),
      numeric("Görselde " + (c.target-100) + " ile " + c.target + " arasındaki fark soruluyor. Fark kaçtır?", 100, [10,99,101], c.target + " - " + (c.target-100) + " = 100.", visual("bars", "Sayı farkı", { items:[{label:"Büyük",value:c.target},{label:"Küçük",value:c.target-100}] }), 10)
    ];
  }

  const measureConfigs = {
    kolay: { moneyA: 35, moneyB: 18, budget: 80, hour: 2, addHour: 3, day: 11, addDay: 8, long: 42, short: 25, share: 24 },
    orta: { moneyA: 64, moneyB: 27, budget: 120, hour: 3, addHour: 4, day: 16, addDay: 12, long: 75, short: 38, share: 36 },
    zor: { moneyA: 86, moneyB: 49, budget: 180, hour: 4, addHour: 5, day: 18, addDay: 13, long: 96, short: 47, share: 48 }
  };
  ...
  const operationConfigs = {
    kolay: { a: 126, b: 53, c: 28, groups: 4, each: 6, start: 15, step: 5 },
    orta: { a: 247, b: 128, c: 65, groups: 6, each: 7, start: 32, step: 8 },
    zor: { a: 468, b: 237, c: 124, groups: 8, each: 9, start: 47, step: 12 }
  };
  ...
  const builders = {
    "sayilar-ve-nicelikler-1": numbersOneGrade3,
  ...
  Object.keys(builders).forEach(function (topic) {
    difficulties.forEach(function (difficulty) {
      const slug = "3-sinif-matematik-" + topic + "-" + difficulty + "-test-1";
      const questions = builders[topic](difficulty).map(function (question, index) {
        question.id = index + 1;
        question.question = "3. sınıf " + difficulty + ": " + question.question;
        if (question.visual) {
