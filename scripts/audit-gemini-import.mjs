...
Kalan ${report.manualReview} soru otomatik reddedilmiş sayılmaz. Öğretmen/editör onayı, yaş düzeyi ve gerekli özgün SVG üretimi tamamlanmadan runtime bankasına alınmaz. Kaynak Markdown dosyaları runtime'a bağlanmaz ve ZIP repository'ye kopyalanmaz. Ayrıntılı soru/uyarı listesi Git dışı \`outputs/gemini-import-audit.json\` dosyasındadır.
`);
console.log(JSON.stringify({ sourceFiles: report.sourceFiles, questions: report.questions, accepted: report.accepted, rejected: report.rejected, manualReview: report.manualReview, missingImages: report.missingImages }));
